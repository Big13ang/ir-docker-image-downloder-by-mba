...
- [شروع سریع](#شروع-سریع)
- [دانلود image خصوصی از GHCR](#دانلود-image-خصوصی-از-ghcr)
...
## دانلود image خصوصی از GHCR

برای imageهای خصوصی مثل این:

```text
ghcr.io/big13ang/inshop-pre-registration-back-end:sha-437ac36
```

باید قبل از pull کردن، workflow به GHCR لاگین کند. لطفا توکن را هیچ‌وقت داخل کد، README یا اسم image ننویسید؛ فقط از GitHub Secrets استفاده کنید.

### 1. ساخت توکن

در GitHub یک Personal Access Token بسازید که به package خصوصی دسترسی داشته باشد. برای GHCR معمولا دسترسی `read:packages` لازم است. اگر package به repo خصوصی وصل است، مطمئن شوید همان اکانت به آن repo/package دسترسی دارد.

لینک مستقیم ساخت token:

https://github.com/settings/tokens

### 2. اضافه کردن Secretها

در همین repo بروید به:
...
مقدار آن همان token خصوصی شماست.

### 3. اجرای workflow

در تب **Actions**، workflow با نام **Save Docker Image** را اجرا کنید و این مقدارها را بگذارید:

| Input | مقدار |
| --- | --- |
| `image` | `ghcr.io/big13ang/inshop-pre-registration-back-end:sha-437ac36` |
| `platform` | مثلا `linux/amd64` یا `linux/arm64` |
| `registry_login` | `true` |
| `registry` | `ghcr.io` |
| `compression` | `none` |

اگر platform اشتباه باشد، Docker ممکن است بگوید image برای آن معماری وجود ندارد. در این حالت دوباره workflow را با platform دیگر اجرا کنید.
...
| `registry_login` | قبل از pull به registry خصوصی لاگین کند | `false` |
| `registry` | آدرس registry خصوصی، مثلا `ghcr.io` | `ghcr.io` |
