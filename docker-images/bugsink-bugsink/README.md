...
- [bugsink-bugsink.tar.gz.part-000](https://github.com/Big13ang/ir-docker-image-downloder-by-mba/blob/master/docker-images/bugsink-bugsink/bugsink-bugsink.tar.gz.part-000)
- [bugsink-bugsink.tar.gz.part-001](https://github.com/Big13ang/ir-docker-image-downloder-by-mba/blob/master/docker-images/bugsink-bugsink/bugsink-bugsink.tar.gz.part-001)
...
| Archive file | `bugsink-bugsink.tar.gz` |
| Compression | `gzip` |
| Split | `true` |
| Part count | `2` |
...
- Required compression tool: `gzip`
- Download page: https://gnuwin32.sourceforge.net/packages/gzip.htm
...
```bash
sudo apt update && sudo apt install -y docker.io gzip
```

macOS:

```bash
# gzip is included with macOS by default
```
...
cd bugsink-bugsink
curl -L -o bugsink-bugsink.tar.gz.part-000 https://raw.githubusercontent.com/Big13ang/ir-docker-image-downloder-by-mba/master/docker-images/bugsink-bugsink/bugsink-bugsink.tar.gz.part-000
curl -L -o bugsink-bugsink.tar.gz.part-001 https://raw.githubusercontent.com/Big13ang/ir-docker-image-downloder-by-mba/master/docker-images/bugsink-bugsink/bugsink-bugsink.tar.gz.part-001
...
```bash
cat bugsink-bugsink.tar.gz.part-* > bugsink-bugsink.tar.gz
```

### 5. Load داخل Docker

```bash
gunzip -c bugsink-bugsink.tar.gz | docker load
```
...
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/Big13ang/ir-docker-image-downloder-by-mba/master/docker-images/bugsink-bugsink/bugsink-bugsink.tar.gz.part-000" -OutFile "bugsink-bugsink.tar.gz.part-000"
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/Big13ang/ir-docker-image-downloder-by-mba/master/docker-images/bugsink-bugsink/bugsink-bugsink.tar.gz.part-001" -OutFile "bugsink-bugsink.tar.gz.part-001"
...
```powershell
Get-FileHash -Algorithm SHA256 bugsink-bugsink.tar.gz.part-*
```

### 4. ساخت archive اصلی از قطعه‌ها

```powershell
$out = [System.IO.File]::Create("bugsink-bugsink.tar.gz")
Get-ChildItem -Filter "bugsink-bugsink.tar.gz.part-*" | Sort-Object Name | ForEach-Object {
...
```powershell
gzip -d -c bugsink-bugsink.tar.gz | docker load
```
...
Generated at: `2026-05-11T20:37:26Z`
